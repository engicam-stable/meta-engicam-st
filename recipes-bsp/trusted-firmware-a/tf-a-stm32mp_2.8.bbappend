FILESEXTRAPATHS:prepend := "${THISDIR}/${PN}:"

SRC_URI += "file://0001-stm32mp2-Initial-EDIMM-dtb-commit-boot-and-porting-e.patch \
            file://0002-stm32mp2-Fixed-pwr-pins-pinctrl-and-cleaning-pmic-no.patch \
            file://0003-stm32mp2-enable-sdmmc1.patch \
            file://0004-stm32mp2-SmarCore-Porting-for-XTouch2.0-board.patch \
            file://0005-stm32mp2-iCore-Added-device-tree-for-mask-D.patch \
            file://0006-stm32mp2-MicroGEA-Porting-for-EDIMM2.0-starterkit.patch \
            file://0007-stm32mp2-iCore-disabled-unsupported-low-power-modes.patch \
            file://0008-stm32mp2-SmarCore-porting-version-D-on-XTouch2.0-boa.patch \
            "

#EXTRA_OEMAKE += "DYN_DISABLE_AUTH=1"
ST_TF_A_LOG_LEVEL_DEBUG = "40"